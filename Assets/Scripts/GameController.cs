...
namespace Assets.Scripts
{
    ...
    public class GameController : MonoBehaviour
    {
        ...
        void InitGame()
        {
            ...
            {
                spawnPosition = new Vector3(Random.Range(0, Width), Random.Range(0, Height), -5);
				//spawnPosition = new Vector3(120, 102, 0);
				Instantiate(macrophage, spawnPosition, spawnRotation);
            }

            for (int nb = 0; nb < Parameter.NumberOfBacteria; nb++)
            {
				var x = GaussianRandom(Width / 2, Width / 4);
				var y = GaussianRandom(Height / 2, Height / 4);
		...
		void ShootTopedo()
		{
			...
			Instantiate(torpedoObject, RocketStartPoint.position , RocketStartPoint.rotation);            
		}
		...
		IEnumerator DoubleBacteria()
		{
			while (true)
			{
				...
				{
					var b = bactList[i];
					Vector3 spawnPosition = new Vector3(b.transform.position.x+Random.Range(-0.03f,0.03f), b.transform.position.y+Random.Range(-0.03f,0.03f),b.transform.position.z-0.1f );
					Quaternion spawnRotation = Quaternion.identity;
